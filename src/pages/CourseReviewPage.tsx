          ...
          <nav className="flex items-center gap-1 overflow-x-auto py-2" aria-label="Section navigation">
            {TOC_SECTIONS.map((s) => (
              <a
                key={s.id}
                href={`#${s.id}`}
                className="font-mono text-sm text-muted-foreground hover:text-foreground px-3 py-1.5 rounded whitespace-nowrap border-2 border-dashed border-transparent hover:border-border"
              >
                {s.label}
              </a>
            ))}
